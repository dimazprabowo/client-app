<div x-data="{ open: false, loadingId: null }" @click.outside="open = false" class="relative">
    {{-- Bell Button --}}
        ...
        <div class="max-h-80 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-700">
            @forelse($notifications as $notification)
                <div wire:key="notif-{{ $notification->id }}"
                     ...
                     @click="loadingId = {{ $notification->id }}; await $wire.markAsRead({{ $notification->id }}); @if($notification->action_url) Livewire.navigate('{{ $notification->action_url }}'); @endif"
                     :class="{ 'opacity-60': loadingId === {{ $notification->id }} }">
                    <div class="flex items-start space-x-3">
                        {{-- Icon --}}
                        <div class="flex-shrink-0 mt-0.5">
                            <div x-show="loadingId !== {{ $notification->id }}" class="w-8 h-8 rounded-full flex items-center justify-center @switch($notification->type) @case('success') bg-green-100 dark:bg-green-900/30 @break @case('warning') bg-yellow-100 dark:bg-yellow-900/30 @break @case('danger') bg-red-100 dark:bg-red-900/30 @break @default bg-blue-100 dark:bg-blue-900/30 @endswitch">
                            ...
                            <div x-show="loadingId === {{ $notification->id }}" class="w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center">
